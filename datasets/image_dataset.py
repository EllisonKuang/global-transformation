...
class SingleImageDataset(Dataset):
    def __init__(self, cfg):
        ...
        self.ill_img = Image.open(cfg["ill_image_path"]).convert("RGB")
        if cfg["resize_input"] > 0:
            self.src_img = transforms.Resize(cfg["resize_input"])(self.src_img)
            self.ill_img = transforms.Resize(cfg["resize_input"])(self.ill_img)
        self.step = -1
    ...
    def __getitem__(self, index):
        ...
        if self.step % self.cfg["source_image_every"] == 0:
            ...
            sample["illu_input_image"] = self.base_transforms(self.ill_img)

        sample["input_crop"] = self.input_transforms(self.src_img)
        sample["illu_input_crop"] = self.base_transforms(self.ill_img)
