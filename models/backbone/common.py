...
class DecorrelatedColorsToRGB(nn.Module):
    ...
    def forward(self, image):
        # if image.dim() == 4:
        image_rgb, alpha, att, illu_rgb = image[:, :3], image[:, 3].unsqueeze(1), image[:, 4].unsqueeze(1), image[:, 5:8]
        image_rgb = torch.einsum("nchw,cd->ndhw", image_rgb, self.colcorr_t)
        illu_rgb = torch.einsum("nchw,cd->ndhw", illu_rgb, self.colcorr_t)
        image = torch.cat([image_rgb, alpha, att, illu_rgb], dim=1)
        # else:
